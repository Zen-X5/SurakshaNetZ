...
export class SessionService {
  ...
  generateJwt(payload: { id: string; displayName?: string}): string {
    return this.jwtService.sign(payload);
  }

  getTokenExpiryTime(duration: number): Date {
  ...
  async generateSession(
    ...
    try {
      ...
      const newAccessToken = this.generateJwt({
        id: user._id.toString(),displayName:user.displayName});
      this.logger.debug({
      ...
      const expiresAt = this.getTokenExpiryTime(30 * 24 * 60 * 60 * 1000);
